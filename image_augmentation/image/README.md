...
| **SolarizeAdd** | Add certain intensity `magnitude` to each pixel and invert all pixels above a threshold value. | ✅ Implemented | Compare results of applying the operation with no additive value on a random image with `PIL.ImageOps.solarize`. | Failing |
